...
RelatedFiles:
    ...
    - Path: vm-system/vm-system/internal/web/spa.go
      Note: SPA fallback and `/api` bypass implementation.
    - Path: vm-system/vm-system/internal/web/generate.go
      Note: `go generate` bridge entrypoint for UI build + copy.
    - Path: vm-system/vm-system/internal/web/tools/main.go
      Note: Build-and-copy generator implementation.
    - Path: vm-system/vm-system/Makefile
      Note: Merged developer workflow targets.
    ...
    - Path: vm-system/vm-system/pkg/vmtransport/http/server.go
      Note: |-
        ...
        Backend API route contract consumed by the UI
ExternalSources: []
Summary: Design + implementation record for merging vm-system-ui into vm-system, including executed subtree import, daemon static serving integration, asset generation bridge, and developer workflow updates.
LastUpdated: 2026-02-14T12:45:00-05:00
WhatFor: Capture both planned and implemented integration strategy for backend+frontend consolidation into one repository/runtime.
WhenToUse: Use when reviewing what was implemented in VM-017, validating deployment/build behavior, or planning follow-up optimization tickets.
---
...
1. Imported `vm-system-ui` into `vm-system/ui` with preserved history via `git subtree`.
2. Added Go-side static web serving package (`internal/web`) with:
- disk mode (`!embed`) and embed mode (`embed`) asset loading,
- SPA fallback routing that bypasses `/api` traffic.
3. Wired `cmd serve` to compose API handler + SPA handler with graceful API-only fallback when assets are unavailable.
4. Added `go generate` bridge to build UI and copy assets into `internal/web/embed/public`.
5. Added root `Makefile` targets for merged dev/build operations.
6. Validated:
- `GOWORK=off go test ./...`
- `pnpm -C ui check`
- `pnpm -C ui run build`
...
7. Removed imported Manus/debug artifacts (`__manus__` runtime collector and related Vite/plugin wiring).

Primary implementation commits:

- `ed8d9dc` — subtree import commit generated by `git subtree add`
- `fa4335a` — daemon SPA/static serving integration
- `fda8502` — `go generate` UI build/copy bridge
- `b2b4115` — merged workflow `Makefile`
- `9371ffa` — Manus/debug artifact cleanup
...
- `vm-system/internal/web/spa.go:1`
- `vm-system/internal/web/generate.go:1`
- `vm-system/internal/web/tools/main.go:1`
