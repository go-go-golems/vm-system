...
- [ ] Finalize terminology contract for this ticket: template-centric naming only, no compatibility aliases (`vm`, `vm-id`, legacy modules command) in user-facing surfaces
- [ ] Add/extend template API endpoints for module and library mutation/query so CLI no longer needs direct DB writes for these operations
