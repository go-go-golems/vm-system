...
RelatedFiles:
    ...
    - Path: client/src/lib/pluginSandboxClient.ts
      Note: Current in-process sandbox client with window.definePlugin behavior
    ...
    - Path: client/src/workers/pluginSandbox.worker.ts
      Note: Current worker implementation still using new Function and contract drift
    - Path: package.json
