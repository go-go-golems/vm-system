---
Title: Getting started from first VM to contributor workflow
...
RelatedFiles:
    - Path: README.md
      Note: Baseline daemon-first quickstart and API surface
    - Path: cmd/vm-system/main.go
      Note: CLI root command tree and global flags
    - Path: cmd/vm-system/cmd_serve.go
      Note: Daemon startup command and lifecycle wiring
    - Path: cmd/vm-system/cmd_template.go
      Note: Template CLI behavior and output conventions
    - Path: cmd/vm-system/cmd_session.go
      Note: Session lifecycle CLI behavior
    - Path: cmd/vm-system/cmd_exec.go
      Note: Execution CLI behavior and event rendering
    - Path: pkg/vmdaemon/app.go
      Note: Long-lived daemon process host around core runtime
    - Path: pkg/vmcontrol/core.go
      Note: Transport-agnostic core wiring of template/session/execution services
    - Path: pkg/vmtransport/http/server.go
      Note: REST API endpoint surface and error mapping
    - Path: pkg/vmstore/vmstore.go
      Note: SQLite schema and persistence implementation
    - Path: pkg/vmsession/session.go
      Note: In-memory runtime session manager and startup/library loading
    - Path: pkg/vmexec/executor.go
      Note: REPL/run-file execution behavior and event generation
    - Path: test-e2e.sh
      Note: Daemon-first full workflow script
    - Path: smoke-test.sh
      Note: Fast confidence test for build plus core runtime path
    - Path: pkg/vmtransport/http/server_templates_integration_test.go
      Note: Template endpoint integration coverage
    - Path: pkg/vmtransport/http/server_sessions_integration_test.go
      Note: Session endpoint lifecycle and runtime summary coverage
    - Path: pkg/vmtransport/http/server_executions_integration_test.go
      Note: Execution endpoint lifecycle and event filtering coverage
    - Path: pkg/vmtransport/http/server_error_contracts_integration_test.go
      Note: 400/404/409/422 API contract coverage
    - Path: pkg/vmtransport/http/server_safety_integration_test.go
      Note: Path traversal and output/event limit enforcement tests
ExternalSources: []
Summary: End-to-end onboarding and implementation guide for vm-system, from first daemon run through architecture, testing strategy, and practical contribution workflow.
LastUpdated: 2026-02-08T12:10:00-05:00
WhatFor: Help new developers become productive quickly and then operate confidently inside vm-system internals without guesswork.
WhenToUse: Use when starting work on vm-system, onboarding teammates, planning backend changes, or preparing production-grade contributions.
---
